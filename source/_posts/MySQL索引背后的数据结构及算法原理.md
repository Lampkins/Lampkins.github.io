...
![img](MySQL索引背后的数据结构及算法原理/8.png)

<p align="center">图8</p>

这里设表一共有三列，假设我们以Col1为主键，则图8是一个MyISAM表的主索引（Primary key）示意。可以看出MyISAM的索引文件仅仅保存数据记录的地址。在MyISAM中，主索引和辅助索引（Secondary key）在结构上没有任何区别，只是主索引要求key是唯一的，而辅助索引的key可以重复。如果我们在Col2上建立一个辅助索引，则此索引的结构如下图所示：

![img](MySQL索引背后的数据结构及算法原理/9.png)
...
![img](MySQL索引背后的数据结构及算法原理/10.png)
...
![img](MySQL索引背后的数据结构及算法原理/11.png)
...
![img](MySQL索引背后的数据结构及算法原理/12.png)
...
![img](MySQL索引背后的数据结构及算法原理/13.png)
...
![img](MySQL索引背后的数据结构及算法原理/14.png)
...
<script>
    ...
    for (let img of imgs) {
        img.setAttribute('class', 'fancybox center-block');
    }
</script>